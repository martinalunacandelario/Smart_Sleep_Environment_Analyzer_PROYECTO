...
async function pollStatus() {
  try {
    ...
    if (_lastStatus !== null && _lastStatus !== d.sessionActive) { loadLists(); }
...
async function showDetail(sid) {
  ...
  try {
    ...
    var igHtml = '<div class="ig-grid">'
      ...
      +'</div>';
    var statsHtml = '<p>CO₂ → máx <strong>'+(stats.co2&&stats.co2.max!==undefined?stats.co2.max:'--')+'</strong> · mín <strong>'+(stats.co2&&stats.co2.min!==undefined?stats.co2.min:'--')+'</strong> · promedio <strong>'+(stats.co2&&stats.co2.avg!==undefined?stats.co2.avg:'--')+'</strong> ppm</p>'
   ...
   var bestHour = '--';
    ...
    if (stats.recommendations && stats.recommendations.length > 0) {
      ...
      alerts.alerts.forEach(function(a){ var msg='', t=(a.type||'').toLowerCase();
    ...
    } else { recsHtml = '<p style="color:#8880a0;font-size:13px">Sin recomendaciones — las condiciones fueron óptimas durante toda la sesión.</p>'; }
   
    document.getElementById('modal-body').innerHTML = '<h2>Detalle de sesión #'+sid+'</h2>'
...
void WebServerTask::handleApiStatus() {
    ...
    if (snap.co2 > 0) {
        ...
        if (snap.temperature < TEMP_GOOD_MIN || snap.temperature > TEMP_ACCEPTABLE_MAX) malo = true;
        else if (snap.temperature > TEMP_GOOD_MAX) regular = true;
        if (snap.humidity < HUM_ACCEPTABLE_MIN1 || snap.humidity > HUM_ACCEPTABLE_MAX2) malo = true;
        else if ((snap.humidity >= HUM_ACCEPTABLE_MIN1 && snap.humidity < HUM_GOOD_MIN) || (snap.humidity > HUM_GOOD_MAX && snap.humidity <= HUM_ACCEPTABLE_MAX2)) regular = true;
...
void WebServerTask::handleApiSessionAlerts() {
    ...
    if (alertsPath.length() > 0) {
        f = SD.open(alertsPath.c_str(), FILE_READ);
    }
...
void WebServerTask::handleApiSessionData() {
    ...
    if (root) {
        ...
        while (f) {
            ...
            String name = sdBasename(fullPath.c_str());
            f.close();
            if (name.startsWith(prefix) && name.indexOf("_alerts") < 0 && name.indexOf("_stats") < 0 && name.endsWith(".csv")) {
