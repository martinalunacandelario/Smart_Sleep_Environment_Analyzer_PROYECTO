                    ...
                    if (readSessionFile(filename, stats)) {
                        ...
                        if (!stats.bestHourValid) {
                            ...
                            Serial.printf("Mejor franja (relativa): %lu:%02lu - %lu:%02lu\n", 
                                          stats.bestHourStart / 3600, (stats.bestHourStart % 3600) / 60,
                                          stats.bestHourEnd / 3600, (stats.bestHourEnd % 3600) / 60);
                        }
                        Serial.println("========================================\n");
                    } else {
...
void AnalysisTask::saveStatisticsToSD(const SessionStats &stats) {
    ...
    if (!stats.bestHourValid) {
        ...
        unsigned long startH = stats.bestHourStart / 3600;
        unsigned long startM = (stats.bestHourStart % 3600) / 60;
        unsigned long endH   = stats.bestHourEnd / 3600;
        unsigned long endM   = (stats.bestHourEnd % 3600) / 60;
        
        statsFile.printf("    \"display\": \"+%02lu:%02lu - +%02lu:%02lu (relativo, sin NTP)\"\n",
